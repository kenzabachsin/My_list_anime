// Masukkan ID MyAnimeList di sini. 
// Kamu bisa tambah terus angkanya sesuai list panjangmu tadi.
const animeIds = [
    38790, // Bofuri
    41514, // Bofuri S2
...
// Fungsi ambil data satu per satu agar tidak error (limit API)
async function fetchSemuaAnime() {
    for (const id of animeIds) {
        try {
            ...
            // Kasih jeda dikit biar API nggak marah (rate limit)
    ...
    detailKonten.innerHTML = `
        <div class="detail-header">
            ...
            <div class="info-text">
                ...
                <p><strong>Author/Studio:</strong> ${anime.studios.map(s => s.name).join(', ')}</p>
                <p><strong>Total:</strong> ${anime.type} (${episodes} Eps)</p>
                <p><strong>Score:</strong> ⭐ ${anime.score || 'N/A'}</p>
            </div>
        </div>
        <hr border="0" height="1px" background="#333">
        <h3>Prolog</h3>
        <p>${anime.synopsis || 'Tidak ada deskripsi.'}</p>
    `;
